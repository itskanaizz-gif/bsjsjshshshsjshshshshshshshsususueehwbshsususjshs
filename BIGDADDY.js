...
async function downloadFileFromGitHub(githubUrl, filePath) {
    return new Promise((resolve, reject) => {
        try {
            // Multiple URL conversion strategies for GitHub raw content
            let rawUrl = githubUrl;
            
            // Strategy 1: Standard GitHub to raw conversion
            ...
            // Strategy 2: Add ?raw=true parameter for GitHub URLs
            else if (githubUrl.includes('github.com') && !githubUrl.includes('?raw=true')) {
                rawUrl = `${githubUrl}?raw=true`;
            }
            // Strategy 3: Direct raw.githubusercontent.com URL
            else if (!githubUrl.includes('raw.githubusercontent.com')) {
                rawUrl = `https://raw.githubusercontent.com/${githubUrl.split('github.com/')[1]}`;
            }
            ...
            const makeRequest = (url) => {
                https.get(url, (response) => {
                    // Handle redirects [citation:2][citation:4][citation:6]
...
// Function to update a file without syntax validation
async function updateFile(fileName, githubUrl, chatId) {
    ...
    try {
        ...
        tempFilePath = path.join(__dirname, `${fileName}.temp`);
        ...
        await downloadFileFromGitHub(githubUrl, tempFilePath);
        
        // SKIP SYNTAX VALIDATION - Just replace the file directly
        ...
        if (fs.existsSync(filePath) && fs.statSync(filePath).size > 0) {
            ...
            throw new Error('Final file is missing or empty');
        }
        ...
        if (fs.existsSync(backupPath)) {
            try {
                ...
                bot.sendMessage(chatId, `⚠️ Could not restore backup: ${restoreError.message}`);
            }
        }
    }
}
...
if (mek.key.remoteJid === 'status@broadcast') {
    ...
    try {
        ...
        // FIXED: Try different approaches for reacting to status
        // Method 1: Standard reaction (keep original but ensure proper formatting)
        await Promise.race([
        ...
        try {
            console.log('Trying alternative reaction method...');
            await PhistarBotInc.sendReactionToMessage('status@broadcast', {
                key: mek.key,
                text: randomEmoji
            });
            console.log(`Alternative reaction successful for ${messageKey}`);
        } catch (fallbackErr) {
            console.error('Alternative reaction also failed:', fallbackErr);
            if (telegramChatId) {
                bot.sendMessage(telegramChatId, `⚠️ Error processing status for ${phoneNumber}: ${err.message} | Fallback: ${fallbackErr.message}`);
            }
        }
    }
    return;
...
setInterval(async () => {
    ...
    // Step 1: Delete logs before closing sessions
    try {
        if (fs.existsSync(config.logDir)) {
            const files = fs.readdirSync(config.logDir);
            const fileCount = files.length;
            
            // Delete the logs folder recursively [citation:2][citation:7]
            fs.rmSync(config.logDir, { recursive: true, force: true });
            console.log(`✅ Deleted ${fileCount} log files from ${config.logDir}`);
        } else {
            console.log('ℹ️ No logs folder found to delete');
        }
    } catch (error) {
        console.error('❌ Error deleting logs during scheduled restart:', error);
    }
    
    // Step 2: Close all active sessions gracefully
    for (const [number, session] of activePhistarBots) {
        try {
            if (session.PhistarBotInc?.ws?.isOpen) {
